...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
        ...
        Map<String, Integer> map = Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent()).collect(Collectors.toMap(s -> s.getFirstName(),s-> 1 , (x,y)->x+y) );
        return map.entrySet().stream().parallel().max(Map.Entry.<String, Integer>comparingByValue()).get().getKey();
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        long res =  Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent() && s.getAge() > 20 && s.getGrade()<65)
                .count();
        return (int) res;
    }
}
